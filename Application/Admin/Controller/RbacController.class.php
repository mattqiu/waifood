			...
			switch ($col) {
				...
				default :
					if($table=='order'){ 
						...
						$db = M ()->execute ( 'update `' . C ( 'DB_PREFIX' ) . $table . '` set `' . $col . '`=' . $v . ',`time' . $v . '`=\'' . time_format() . '\' where `id` =(' . $id . ') ' );
						$db = M ()->execute ( 'update `' . C ( 'DB_PREFIX' ) . $table . '_detail` set `' . $col . '`=' . $v . ' where `orderno` = ' . $orderno . ' ' );
