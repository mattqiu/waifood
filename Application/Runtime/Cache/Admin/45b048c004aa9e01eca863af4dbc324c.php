  ...
  <table border="0" cellpadding="3" cellspacing="1" class="MainTbl">
    ...
    <tr>
      <td>订单状态： <a href="<?php echo U('Cms/Order');?>" >全部</a>
        ...
        <?php else: ?>
          <?php if(($key) == $status): ?><a href="<?php echo U('Cms/Order','orderfrom='.$orderfrom.'&status='.$key.'&pay='.$pay);?>" class="fc_red"><?php echo ($type); ?></a>
            <?php else: ?>
            <a href="<?php echo U('Cms/Order','orderfrom='.$orderfrom.'&status='.$key.'&pay='.$pay);?>" ><?php echo ($type); ?></a><?php endif; endif; endforeach; endif; else: echo "" ;endif; ?></td>
    </tr>
    <tr>
      <td>付款状态： <a href="<?php echo U('Cms/Order');?>" >全部</a>
        ...
        <?php if(($pay == '0')): ?><a href="<?php echo U('Cms/Order','orderfrom='.$orderfrom.'&pay=0&status='.$status);?>" class="fc_red">未付款</a>
          <?php else: ?>
          <a href="<?php echo U('Cms/Order','orderfrom='.$orderfrom.'&pay=0&status='.$status);?>">未付款</a><?php endif; ?></td>
    </tr>
    <tr>
      <td>订单来源： <a href="<?php echo U('Cms/Order');?>" >全部</a>
        ...
        <?php if(($orderfrom == '0')): ?><a href="<?php echo U('Cms/Order','orderfrom=0&pay='.$pay.'&status='.$status);?>" class="fc_red">网站</a> 
          <?php else: ?>
        <a href="<?php echo U('Cms/Order','orderfrom=0&pay='.$pay.'&status='.$status);?>">网站</a><?php endif; ?></td>
    </tr>
  </table>
</form>
	<div class="dot"></div>
	<table border="0" cellpadding="3" cellspacing="1" class="MainTbl">
		...
		<tr class="row0">
			<td colspan="10"><a href="<?php echo U('Admin/Cms/addOrder');?>"
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="row<?php echo ($i % 2+1); ?>">
			<td><input type="checkbox" name="SelectIDs" value="<?php echo ($vo["id"]); ?>" />
				<?php echo ($vo["id"]); ?></td>
			<td><?php echo ($vo["addtime"]); ?><br /><?php if(($vo["orderfrom"]) == "1"): ?><span class="fc_red" title="该订单来自微信">微</span><?php endif; ?></td>
			<td><?php echo ($vo["delivertime"]); ?></td>
			<td>
            <?php if(($vo["usertype"]) == "2"): echo ($vo["username"]); ?>
            <?php else: ?>
            <?php echo get_username($vo['userid']); endif; ?> <br /><?php echo ($vo["telephone"]); ?></td>
			<td> <?php echo ($vo["amount"]); ?><br />
                      ...
                      <?php case "2": ?><span class="fc_red">物+洗</span><?php break; endswitch;?></td>
			<td><?php if(($vo["invoice"]) == "1"): ?>是
                    <?php else: ?>  
                    否<?php endif; ?>
                      </td>
			<td><?php if(($vo["paymethod"]) == "4"): ?>货到付款
                    <?php else: ?>  
                    在线支付<?php endif; ?>
                      <br /><?php if(($vo["pay"]) == "0"): ?><span class="fc_blue">未支付</span>
                    <?php else: ?>  
                    <span class="fc_red">已支付</span><?php endif; ?></td>
