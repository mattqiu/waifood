		...
		if (in_array($col, $cols)) {
			switch ($col) {
				case '__del__' :
					...
					$db = M ()->query ( 'delete from `' . C ( 'DB_PREFIX' ) . $table . '`   where `id` in (' . $id . ') ' );
				...
				case '__sta__' :
					...
					$db = M ()->query ( 'update `' . C ( 'DB_PREFIX' ) . $table . '`  set `status`=' . $v . '  where `id` in (' . $id . ') ' );
				...
				default :
					if($table=='order'){ 
						...
						if($order['status']==3 || $v==4){
						    withdrawOrder($orderno);
						}
						$db = M ()->query ( 'update `' . C ( 'DB_PREFIX' ) . $table . '` set `' . $col . '`=' . $v . ',`time' . $v . '`=\'' . time_format() . '\' where `id` =(' . $id . ') ' );
						$db = M ()->query ( 'update `' . C ( 'DB_PREFIX' ) . $table . '_detail` set `' . $col . '`=' . $v . ' where `orderno` = ' . $orderno . ' ' );
						
						if($v==3){
						    finishOrder($orderno);
							$where = array ();
							$where ['orderno'] = $orderno;
							$db = M ( 'order' )->field ( 'userid,amount,paymethod' )->where ( $where )->find ();
						// 增加积分
						...
						$db = M ()->query ( 'update `' . C ( 'DB_PREFIX' ) . $table . '` set `' . $col . '`=' . $v . ' where `id` in (' . $id . ') ' );
					}
					break;
			}
			if ($db !== false) {
