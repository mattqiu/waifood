...
class PointController extends BaseController {
    ...
    public function lists($id = 0,$order=1,$price='',$brand='') {
    	...
    	$rs = M ( "product" )->field('id,title,indexpic,price,unit')->where ( $where )->order ( $orderstr )->page ( $p, $row );
    	$list = $rs->select ();
    	...
    	$cateinfo=M('category')->field('id,pid')->where('id='.$id)->find();
    	...
    	$where['pid']= $id;
    	$where['status']= 1;
    	$brandArr=M('product')->where($where)->distinct(true)->getField('brand',true); 
    	$this->assign('brandlist',$brandArr);
    	
    	//3. 价格区间
    	$priceArr=array(1,100,200,300,500,1000); 
    	$newarr=array();
    ...
    public function view($id=0){
    	$where=array();
    	$where['status']=1;
    	$db = M ( "product" )->where($where)->find ($id);
    	if($db){
