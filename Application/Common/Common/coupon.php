...
function is_address() {
	...
	if ($db) {
		if (isN ( $db ['telephone'] ) || isN ( $db ['address'] )) {
			return false;
		} else {
			return $db;
		}
...
function get_my_coupon() {
	...
	$datefrom='';
	$list = M ( 'coupon' )->where ( $where )->field ( 'id,amount,coupontype,addtime' )->order('timefrom asc')->select ();
	if ($list) {
		$total = 0;
		foreach ( $list as $k => $v ) {
		    if($k==0){
		...
		if($datefrom){
		    ...
		    $where ['status'] = 1;
		    $where ['coupontype'] = 0;
		    $where ['addtime'] = array('gt',$datefrom);
		    $count0 = M ( 'coupon' )->where ( $where )->sum('amount');
		  	$total-=$count0;
		}
		if ($total < 0) {
			$total = 0;
